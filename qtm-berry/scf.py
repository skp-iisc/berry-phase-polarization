...
def scf(
    ...
    ret_vxc: bool = False,
    efield_cart: tuple[float, float, float] | None = None,
    kgrid_shape: tuple[int, int, int] | None = None
) -> (
    ...
    with image_comm:
        ...
        def solve_kswfn(kswfn_k: list[KSWfn], ksham_k: list[KSHam], ik_global: int, full_wfn: list):
            numiter = 0
            for ispin in range(2 if is_spin and not is_noncolin else 1):
                kswfn_ = kswfn_k[ispin]
                
                # FORCE update of KSHam if the electric field is active
                if ksham_k[ispin] is None or efield_cart is not None:
                    ksham_k[ispin] = KSHam(
                        kswfn_.gkspc,
                        is_noncolin,
                        vloc if is_noncolin else vloc[ispin],
                        l_nloc,
                        efield_cart=efield_cart, 
                        full_wfn=full_wfn, 
                        kpts=kpts,
                        kgrid_shape=kgrid_shape, 
                        ik=ik_global, 
                        crystal=crystal,
                        current_kswfn=kswfn_
                    )
        ...
        P_el_cart = np.zeros(3)

        def compute_Pel(full_wfn):
            if full_wfn is None or len(full_wfn) == 0: 
                return np.zeros(3)
                
            vol = crystal.reallat.cellvol
            ...
            Gam = np.zeros(3)
            for d in range(3):
                perp_dirs = [x for x in range(3) if x != d]
                N = kgrid_shape
                
                # Construct Strings by rearranging K-points
                strings_dict = {}
                for i in range(nk):
                    k = all_k[:, i]
                    perp_key = tuple(int(np.round(k[x] * N[x] * 2)) % (N[x] * 2) for x in perp_dirs)
                    if perp_key not in strings_dict:
                        strings_dict[perp_key] = []
                    strings_dict[perp_key].append(i)
                    
                Gam_d = 0.0
                for key in sorted(strings_dict.keys()):
                    string = strings_dict[key]
                    string.sort(key=lambda ik: all_k[d, ik] % 1.0)
                    N_str = len(string)
                    ...
                    for j in range(N_str):
                        ...
                        S = full_wfn[ik1].overlap(full_wfn[ik2], range(numbnd_occ), range(numbnd_occ), umklapp)
                        det_prod *= np.linalg.det(S)
                        
                    Gam_d += np.angle(det_prod)
                    
                Gam[d] = Gam_d / len(strings_dict)

            Gam_wrapped = (Gam + np.pi) % (2 * np.pi) - np.pi
            
            # Pure Electronic Polarization
            return -(1.0 / (np.pi * vol)) * sum([Gam_wrapped[i] * latvec[:, i] for i in range(3)])

        def compute_en():
            nonlocal rho_in, rho_out, P_el_cart
            e_eigen = 0
            for kswfn_k in l_kswfn_kgrp:
                e_eigen += sum(
            ...
            E_KS = en.one_el + en.ewald + en.hartree + en.xc
            
            if efield_cart is not None:
                full_wfn_pol = [w[0] for w in l_kswfn_kgrp]
                P_el_cart = compute_Pel(full_wfn_pol)
                en.efield_en = -crystal.reallat.cellvol * np.dot(P_el_cart, efield_cart)
            else:
                en.efield_en = 0.0

            en.total = E_KS + en.efield_en
            en.hwf = np.real(
                e_eigen
        ...
        while idxiter < maxiter:
            ...
            if efield_cart is not None:
                full_wfn = [w[0] for w in l_kswfn_kgrp]
            else:
                full_wfn = None

            diago_avgiter = 0
            for ik_global, (kswfn_, ksham_) in zip(
                i_kpts_kgrp, zip(l_kswfn_kgrp, l_ksham_kgrp)
            ):
                diago_avgiter += solve_kswfn(kswfn_, ksham_, ik_global, full_wfn)
        ...
        if ret_vxc:
            ...
            ret = [scf_converged, rho_in, l_kswfn_kgrp, en, vxc_arr]
        else:
            ret = [scf_converged, rho_in, l_kswfn_kgrp, en]
            
        if efield_cart is not None:
            ret.append(P_el_cart)
